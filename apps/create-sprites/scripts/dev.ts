...
const sourceRoot = resolve(workspaceRoot, "sprites", "ember-scout")
const frontendPort = resolveDevPort({
	argv: process.argv.slice(2),
	defaultPort: DEFAULT_DEV_PORT + 4,
	...(process.env.CREATE_SPRITES_DEV_PORT === undefined ? {} : { environmentValue: process.env.CREATE_SPRITES_DEV_PORT }),
	portCount: 2,
})
...
await superviseDevProcesses([
	...
	spawn(process.execPath, [vpEntrypoint, "dev", "./public", "--config=./vite.config.ts", "--host=127.0.0.1", `--port=${frontendPort}`, "--strictPort"], {
		cwd: packageRoot,
		env: { ...process.env, CREATE_SPRITES_BACKEND_PORT: String(backendPort) },
		stdio: "inherit",
	}),
])
