...
beforeAll(async () => {
	...
	for (const dependency of Object.keys(createFontPackage.dependencies)) {
		...
		await mkdir(resolve(target, `..`), { recursive: true })
		await symlink(
			resolve(workspaceRoot, `apps/create-font/node_modules`, dependency),
			target,
			`junction`,
		)
	}
...
`,
		...
		const editorPackage = JSON.parse(
			await readFile(
				resolve(workspaceRoot, `packages/create-font/editor/package.json`),
				`utf8`,
			),
		) as { version: string }
