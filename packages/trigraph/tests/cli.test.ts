...
describe(`trigraph CLI`, () => {
	it(`renders help from the root route`, async () => {
		const captured = captureIo()
		const exitCode = await runCli([`bun`, `trigraph`], captured.io)

		expect(exitCode).toBe(0)
		expect(captured.stdout.join(``)).toContain(`trigraph`)
	...
	it(`runs the preliminary build command`, async () => {
		...
		const exitCode = await runCli(
			[`bun`, `trigraph`, `build`, `--root`, import.meta.dir],
			captured.io,
		)
