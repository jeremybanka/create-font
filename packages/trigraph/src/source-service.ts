...
async function collectJsonPaths(
	...
	for (const entry of await readdir(directory, { withFileTypes: true })) {
		if (entry.name === `.trigraph`) continue
...
async function recoverTransactions(projectRoot: string): Promise<void> {
	const transactionsRoot = join(projectRoot, `.trigraph`, `transactions`)
				...
				throw validationError([
					{
						...
						unitPath: path,
						path: `$`,
						message: `Source unit ${JSON.stringify(path)} is not part of the Trigraph directory contract.`,
					},
				])
			}
