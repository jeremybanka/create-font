...
import { validateEditorFontSource } from "./codec.ts"
import { analyzeFeaProject } from "./fea.ts"

type PlistValue = string | readonly PlistValue[] | PlistDictionary
...
class GlyphsPlistParser {
	...
	#dictionary(): PlistDictionary {
		...
		this.#expect("{")
		const result: Record<string, PlistValue> = Object.create(null)
		for (;;) {
			this.#skipTrivia()
			if (this.#take("}")) {
				this.#depth -= 1
...
/** Parse a Glyphs.app `.glyphs` file and lower supported authoring data to editor v5. */
export function importGlyphsSource(text: string): GlyphsImportResult {
	...
	if (
		...
		const declaredFormatVersion =
			string(root[".formatVersion"] ?? root.formatVersion) ?? formatVersion
		return {
			ok: false,
			errors: [
				{
