...
const FontInfoReadOnlyContext = createContext(false)
...
export function FontInfo({ readOnly = false, workspace }: FontInfoProps) {
	...
	const setNames = <Key extends keyof Names>(key: Key, value: Names[Key]) => {
		workspace.font.actions.setCoreSource({
			field: `names`,
			value: Object.freeze({ ...names, [key]: value }),
		})
	}
	const setMetadata = <Key extends keyof Metadata>(
		...
		workspace.font.actions.setCoreSource({
			field: `metadata`,
			value: Object.freeze({ ...metadata, [key]: value }),
		})
	}
	const setMetrics = <Key extends keyof Metrics>(
		...
		workspace.font.actions.setCoreSource({
			field: `metrics`,
			value: Object.freeze({ ...metrics, [key]: value }),
		})
	}
	const setStyle = <Key extends keyof Style>(key: Key, value: Style[Key]) => {
		workspace.font.actions.setCoreSource({
			field: `style`,
			value: Object.freeze({ ...style, [key]: value }),
		})
	}
	const setOvershoot = (
	...
	return (
		<font-info className={css.class} aria-readonly={readOnly || undefined}>
			<FontInfoReadOnlyContext.Provider value={readOnly}>
				<info-heading>
					<p>Font source</p>
					<h1>Font Info</h1>
					<span>
				...
				<info-layout>
					{readOnly ? <p role="note">Font information is read-only.</p> : null}
					<info-section>
						<section-heading>
							<heading-copy>
								<h2>Naming</h2>
					...
					<info-section>
						...
						<section-heading>
							<heading-copy>
								...
								<p>
									Permitted rounded-outline depth beyond each alignment line.
								</p>
							</heading-copy>
						</section-heading>
						<field-grid>
			...
			</FontInfoReadOnlyContext.Provider>
		</font-info>
	)
}
...
function TextField({
	...
	const disabled = useContext(FontInfoReadOnlyContext)
	return (
		<text-field>
			<label>
				<span>{label}</span>
